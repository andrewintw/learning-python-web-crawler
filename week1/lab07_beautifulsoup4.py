...
while not found:
    ...
    for row in tr_lines:
        ...
        reply_set      = row.select('.l-listTable__td.l-listTable__td--time')[1]
        reply_username = reply_set.select_one('a').text
        reply_time     = reply_set.select_one('.o-fNotes').text

        print(url, title, publish_username, publish_time, publish_count, reply_username, reply_time)
